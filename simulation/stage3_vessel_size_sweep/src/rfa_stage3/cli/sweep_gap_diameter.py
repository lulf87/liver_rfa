...
from rfa_stage3.io_utils import load_yaml, save_json
from rfa_stage3.metrics import compute_all_metrics
...
def main() -> None:
    ...
    cfg0 = deepcopy(cfg)
    cfg0['project']['case_id'] = 'stage3_novessel_reference'
    ...
    for d in diameters:
        for g in gaps:
            cfg_i = deepcopy(cfg)
            cfg_i['project']['case_id'] = f"stage3_d{str(d).replace('.', 'p')}_g{str(g).replace('.', 'p')}"
    ...
    out_csv = ROOT / 'outputs' / 'tables' / 'stage3_gap_diameter_summary.csv'
    ...
    _heatmap(mat('TCR'), gaps, diameters, 'Tumor coverage across gap and vessel size', 'TCR', ROOT / 'outputs' / 'figures' / 'stage3_TCR_heatmap.png')
    _heatmap(mat('MDI'), gaps, diameters, 'Margin deficit across gap and vessel size', 'MDI', ROOT / 'outputs' / 'figures' / 'stage3_MDI_heatmap.png')
    _heatmap(mat('VUA_deg'), gaps, diameters, 'Vessel-side undercoverage across gap and vessel size', 'VUA (deg)', ROOT / 'outputs' / 'figures' / 'stage3_VUA_heatmap.png')
    _heatmap(mat('equivalent_lesion_diameter_mm'), gaps, diameters, 'Equivalent lesion diameter across gap and vessel size', 'Diameter (mm)', ROOT / 'outputs' / 'figures' / 'stage3_eqdiam_heatmap.png')
    _contour_panel(panel_items, ROOT / 'outputs' / 'figures' / 'stage3_contour_panel_gap0.png')
